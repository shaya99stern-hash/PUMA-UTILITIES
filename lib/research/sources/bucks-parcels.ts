...
export type BucksParcelRecord = {
  parcelNumber?: string; address?: string; municipality?: string; owner1?: string; owner2?: string; careOf?: string;
  landValue?: number; buildingValue?: number; totalValue?: number; landUseCode?: string; modifiedAt?: string;
};
...
export function isLikelyBucksCountyAddress(address: string): boolean {
  if (!/\bPA\b/i.test(address)) return false;
  return /\b(?:Bensalem|Bristol|Buckingham|Chalfont|Churchville|Croydon|Doylestown|Dublin|Erwinna|Fairless Hills|Feasterville(?:-Trevose)?|Furlong|Hulmeville|Jamison|Kintnersville|Langhorne|Levittown|Morrisville|New Hope|Newtown|Ottsville|Penndel|Perkasie|Pipersville|Quakertown|Richboro|Riegelsville|Sellersville|Southampton|Spinnerstown|Trevose|Trumbauersville|Tullytown|Upper Black Eddy|Warminster|Warrington|Washington Crossing|Yardley)\b/i.test(address);
}
...
export async function lookupBucksParcelByAddress(address: string, signal?: AbortSignal): Promise<BucksParcelRecord | undefined> {
  ...
  const response = await fetch(url, { headers:{ Accept:'application/json', 'User-Agent':'PumaUtilitiesResearch/1.7 public property research' }, cache:'no-store', signal });
  ...
  const rows = (payload.features ?? []).map((feature) => toRecord(feature.attributes ?? {})).filter((record) => normalizeBucksParcelStreet(record.address ?? '') === street);
  return rows.length === 1 ? rows[0] : undefined;
}

export function ingestBucksParcel(graph: ResearchGraph, propertyId: string, record: BucksParcelRecord, sourceUrl = BUCKS_LAYER, observedAt = new Date().toISOString()): void {
  ...
  addEvidence(graph, { id:evidenceId, sourceId:'pa-county-assessment', url:sourceUrl, observedAt, authority:'official', confidence:0.96, excerpt:[parcel ? `Bucks parcel ${parcel}` : undefined, record.address, record.municipality, record.owner1 ? `owner: ${record.owner1}` : undefined, record.owner2 ? `co-owner: ${record.owner2}` : undefined, record.totalValue ? `assessed total value: ${record.totalValue}` : undefined, record.landUseCode ? `land use ${record.landUseCode}` : undefined].filter(Boolean).join(' · ').slice(0,700) });
  for (const ownerName of [...new Set([record.owner1, record.owner2].map((value) => value?.trim()).filter((value): value is string => Boolean(value)))]) {
    ...
    addClaim(graph, { id:`claim:${propertyId}:owner:bucks:${token(ownerName)}`, subjectId:propertyId, fact:'property.owner', objectEntityId:ownerId, state:'VERIFIED', confidence:0.96, evidenceIds:[evidenceId], observedAt });
  }
}

function toRecord(row: Record<string, unknown>): BucksParcelRecord { return { parcelNumber:text(row.PARCEL_NUM), address:text(row.ADDRESS), municipality:text(row.MUNICIPALITY), owner1:text(row.OWNER1), owner2:text(row.OWNER2), careOf:text(row.CARE_OF), landValue:numberValue(row.LAND_VALUE), buildingValue:numberValue(row.BUILDING_VALUE), totalValue:numberValue(row.TOTAL_VALUE), landUseCode:text(row.LAND_USE_CODE), modifiedAt:dateValue(row.MODIFY_DATE) }; }
...
function dateValue(value: unknown): string | undefined { const parsed = typeof value === 'number' ? new Date(value) : typeof value === 'string' ? new Date(value) : undefined; return parsed && !Number.isNaN(parsed.getTime()) ? parsed.toISOString() : undefined; }
